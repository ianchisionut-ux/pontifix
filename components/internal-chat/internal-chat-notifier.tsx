...
export function InternalChatNotifier(){
  ...
  useEffect(()=>{
    ...
    const firstPoll=setTimeout(poll,2200);const timer=setInterval(poll,10000);return()=>{cancelled=true;clearTimeout(firstPoll);clearInterval(timer);if(toastTimer)clearTimeout(toastTimer)}
  },[])
