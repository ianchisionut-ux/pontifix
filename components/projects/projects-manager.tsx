  ...
  return <article className="card overflow-hidden">
    <div className="p-5">
      <div className="flex flex-wrap gap-4 justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">{editing&&<><select className="input-field !py-2" value={project.status} onChange={event=>patchProject(project.id,{status:event.target.value})}>{Object.entries(PROJECT_LABELS).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><button className="round-action" onClick={edit} title="Editează proiectul"><Pencil size={15}/></button><button className="round-action text-rose-600" onClick={()=>deleteProject(project)} title="Șterge proiectul"><Trash2 size={15}/></button></>}<button className="round-action text-blue-700" onClick={toggle} title={open?'Restrânge':'Vezi detaliile'}>{open?<ChevronUp size={17}/>:<ChevronDown size={17}/>}</button></div>
      </div>
      <div className="mt-4 flex items-center gap-3"><div className="h-2.5 flex-1 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-600 transition-all" style={{width:progress+'%'}}/></div><strong className="text-sm text-blue-700">{progress}%</strong></div>
      <div className="mt-2 flex flex-wrap gap-4 text-xs text-slate-500"><span>{project.approvals.length} avize</span><span>{project.approvals.filter((approval:Approval)=>approval.status==='OBTAINED').length} obținute</span>{project.documentUrl?<a href={`/api/projects/${project.id}/document`} target="_blank" className="text-blue-700 font-semibold">Deschide certificatul</a>:editing?<label className="text-blue-700 font-semibold cursor-pointer inline-flex gap-1 items-center"><UploadCloud size={13}/> Încarcă certificatul<input type="file" accept="application/pdf" className="hidden" onChange={event=>event.target.files?.[0]&&attachCertificate(event.target.files[0])}/></label>:null}</div>
    </div>
    {open&&<div className="border-t bg-slate-50/60 p-5">
      ...
      <div className="mt-6 pt-5 border-t border-slate-200"><button disabled={!editing||!allObtained} onClick={()=>editing&&allObtained&&patchProject(project.id,{constructionAuthorizationStatus:nextStage(project.constructionAuthorizationStatus as ApprovalStatus)})} className={`w-full min-h-20 rounded-2xl border-2 px-5 text-center transition ${editing&&allObtained?'cursor-pointer':'cursor-default'} ${authorizationClass}`}><span className="block text-base font-black tracking-wide">AUTORIZAȚIA DE CONSTRUIRE</span><span className="block text-xs uppercase mt-1">{allObtained?STAGE_LABELS[project.constructionAuthorizationStatus as ApprovalStatus]:'Disponibilă după obținerea tuturor avizelor'}</span></button></div>
    </div>}
  </article>
}
