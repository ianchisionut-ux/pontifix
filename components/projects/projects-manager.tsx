...
import { BarChart3, Building2, CheckCircle2, ChevronDown, ChevronUp, FileText, Link2, Pencil, Plus, Printer, Save, Search, Trash2, UploadCloud } from 'lucide-react'
...
type Project = { id:string; name:string; certificateNumber:string|null; certificateDate:string|null; beneficiary:string|null; address:string|null; description:string|null; status:ProjectStatus; constructionAuthorizationStatus:ApprovalStatus; documentUrl:string|null; documentName:string|null; updatedAt:string; approvals:Approval[] }
...
const STAGE_CLASSES:Record<ApprovalStatus,string> = { REQUIRED:'bg-slate-950 text-white border-slate-950', SUBMITTED:'bg-amber-400 text-amber-950 border-amber-400', OBTAINED:'bg-emerald-500 text-white border-emerald-500', NOT_REQUIRED:'bg-slate-200 text-slate-500 border-slate-200' }
...
export function ProjectsManager({ initialProjects }: { initialProjects:Project[] }) {
  ...
  const [expanded,setExpanded] = useState<string|null>(initialProjects[0]?.id || null)
  const [modal,setModal] = useState<Project|'new'|null>(null)
  const [certificateFile,setCertificateFile] = useState<File|null>(null)
  const [busy,setBusy] = useState(false)

  const visible = useMemo(() => initialProjects.filter(project => (filter === 'ALL' || project.status === filter) && (!query || [project.name,project.beneficiary,project.address].some(value => value?.toLowerCase().includes(query.toLowerCase())))), [initialProjects,filter,query])
  const chart = initialProjects.filter(project => project.status !== 'ARCHIVED').map(project => ({ name:project.name.length>18?project.name.slice(0,18)+'…':project.name, progres:projectProgress(project) }))
  ...
  return <div>
    ...
    {chart.length>0 && <section className="card p-5 mb-5"><h2 className="font-semibold">Stadiul fizic al proiectelor</h2><p className="text-xs text-slate-500 mb-3">Avize 70% · Autorizația de construire 30%</p><div className="h-52"><ResponsiveContainer width="100%" height="100%"><BarChart data={chart}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" fontSize={10}/><YAxis domain={[0,100]} unit="%" fontSize={10}/><Tooltip formatter={value=>`${value}%`}/><Bar dataKey="progres" fill="#2563eb" radius={[7,7,0,0]}/></BarChart></ResponsiveContainer></div></section>}
...
function ProjectCard({project,open,toggle,patchProject,deleteProject,edit,addApproval,patchApproval,deleteApproval,attachApproval,attachCertificate}:any){
  const progress=projectProgress(project); const allObtained=project.approvals.length>0&&project.approvals.every((item:Approval)=>item.status==='OBTAINED')
  return <article className="card overflow-hidden"><div className="p-5"><div className="flex flex-wrap gap-4 justify-between"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-semibold">{project.name}</h2><span className="text-xs rounded-full bg-blue-50 text-blue-700 px-2.5 py-1">{PROJECT_LABELS[project.status as ProjectStatus]}</span></div><p className="text-sm text-slate-500 mt-1">{project.beneficiary||'Beneficiar nespecificat'}{project.address?' · '+project.address:''}</p><p className="text-xs text-slate-400 mt-1">{project.certificateNumber?'CU '+project.certificateNumber:''}{project.certificateDate?' din '+new Date(project.certificateDate).toLocaleDateString('ro-RO'):''}</p></div><div className="flex flex-wrap items-center gap-2"><select className="input-field !py-2" value={project.status} onChange={event=>patchProject(project.id,{status:event.target.value})}>{Object.entries(PROJECT_LABELS).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><button className="round-action" onClick={edit}><Pencil size={15}/></button><button className="round-action text-rose-600" onClick={()=>deleteProject(project)}><Trash2 size={15}/></button><button className="round-action text-blue-700" onClick={toggle}>{open?<ChevronUp size={17}/>:<ChevronDown size={17}/>}</button></div></div><div className="mt-4 flex items-center gap-3"><div className="h-2.5 flex-1 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-600 transition-all" style={{width:progress+'%'}}/></div><strong className="text-sm text-blue-700">{progress}%</strong></div><div className="mt-2 flex flex-wrap gap-4 text-xs text-slate-500"><span>{project.approvals.length} avize</span><span>{project.approvals.filter((a:Approval)=>a.status==='OBTAINED').length} obținute</span>{project.documentUrl?<a href={`/api/projects/${project.id}/document`} target="_blank" className="text-blue-700 font-semibold">Deschide certificatul</a>:<label className="text-blue-700 font-semibold cursor-pointer inline-flex gap-1 items-center"><UploadCloud size={13}/> Încarcă certificatul<input type="file" accept="application/pdf" className="hidden" onChange={event=>event.target.files?.[0]&&attachCertificate(event.target.files[0])}/></label>}</div></div>
  {open&&<div className="border-t bg-slate-50/60 p-5"><p className="text-xs font-bold tracking-wider text-slate-500 mb-3">AVIZELE PROIECTULUI · CLICK PE CULOARE PENTRU SCHIMBAREA STĂRII</p><div className="flex flex-wrap gap-3">{project.approvals.map((approval:Approval)=><div key={approval.id} className="border border-slate-200 rounded-2xl bg-white p-2 flex items-center gap-2"><button onClick={()=>patchApproval(project.id,approval.id,{status:nextStage(approval.status)})} className={`min-h-12 rounded-xl border px-4 text-left ${STAGE_CLASSES[approval.status]}`}><span className="block text-sm font-extrabold">{approval.name}</span><span className="block text-[10px] uppercase tracking-wide opacity-80">{STAGE_LABELS[approval.status]}</span></button>{approval.documentUrl?<a href={`/api/projects/${project.id}/approvals/${approval.id}/document`} target="_blank" className="round-action text-blue-700" title="Deschide avizul"><Link2 size={14}/></a>:<label className="round-action text-blue-700 cursor-pointer" title="Încarcă avizul scanat"><UploadCloud size={14}/><input type="file" accept="application/pdf" className="hidden" onChange={event=>event.target.files?.[0]&&attachApproval(project.id,approval.id,event.target.files[0])}/></label>}<button className="text-rose-500 p-1" onClick={()=>deleteApproval(project.id,approval.id)}><Trash2 size={14}/></button></div>)}</div>
  <form action={(form)=>addApproval(project.id,form)} className="flex flex-wrap gap-2 mt-4"><input name="name" className="input-field flex-1 min-w-56" placeholder="Alt aviz necesar" required/><input name="institution" className="input-field flex-1 min-w-48" placeholder="Instituție (opțional)"/><button className="btn-secondary inline-flex gap-2 items-center"><Plus size={15}/> Adaugă aviz</button></form>
  <div className="mt-6 pt-5 border-t border-slate-200"><button disabled={!allObtained} onClick={()=>allObtained&&patchProject(project.id,{constructionAuthorizationStatus:nextStage(project.constructionAuthorizationStatus as ApprovalStatus)})} className={`w-full min-h-20 rounded-2xl border-2 px-5 text-center transition ${allObtained?STAGE_CLASSES[project.constructionAuthorizationStatus as ApprovalStatus]:'bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed'}`}><span className="block text-base font-black tracking-wide">AUTORIZAȚIA DE CONSTRUIRE</span><span className="block text-xs uppercase mt-1">{allObtained?STAGE_LABELS[project.constructionAuthorizationStatus as ApprovalStatus]:'Disponibilă după obținerea tuturor avizelor'}</span></button></div></div>}
  </article>
}
