...
type Project = { id:string; name:string; certificateNumber:string|null; certificateDate:string|null; beneficiary:string|null; beneficiaryPhone:string|null; address:string|null; description:string|null; status:ProjectStatus; contractSigned:boolean; isTender:boolean; constructionAuthorizationStatus:ApprovalStatus; documentUrl:string|null; documentName:string|null; authorizationDocumentUrl:string|null; authorizationDocumentName:string|null; updatedAt:string; approvals:Approval[] }
...
export function ProjectsManager({ initialProjects, canManage, canSendWhatsApp }: { initialProjects:Project[]; canManage:boolean; canSendWhatsApp:boolean }) {
  ...
  async function createProject(form:FormData){
    ...
    try{
      ...
      const payload:any={...Object.fromEntries(form),isTender:form.getAll('isTender').includes('true')}; delete payload.standardApproval; delete payload.customApproval
...
function ProjectCard({project,compact,editing,canSendWhatsApp,open,toggle,patchProject,deleteProject,edit,addApproval,patchApproval,deleteApproval,attachApproval,attachAuthorization,attachCertificate,sendUpdate,sending}:any){
  ...
  return <article className="card w-full min-w-0 overflow-hidden">
    <div className="px-4 py-3.5">
      ...
      <div className="mt-1.5 flex flex-wrap gap-3 text-[11px] text-slate-500"><span>{project.approvals.length} avize</span><span>{project.approvals.filter((approval:Approval)=>approval.status==='OBTAINED').length} obținute</span>{project.documentUrl?<a href={`/api/projects/${project.id}/document`} target="_blank" className="text-blue-700 font-semibold">Deschide certificatul</a>:editing?<label className="text-blue-700 font-semibold cursor-pointer inline-flex gap-1 items-center"><UploadCloud size={13}/> Încarcă certificatul<input type="file" accept="application/pdf" className="hidden" onChange={event=>event.target.files?.[0]&&attachCertificate(event.target.files[0])}/></label>:null}</div>{compact&&<div className="mt-2.5 flex flex-wrap items-center gap-1.5">{project.approvals.map((approval:Approval)=><button key={approval.id} type="button" disabled={!editing} onClick={()=>editing&&patchApproval(project.id,approval.id,{status:nextStage(approval.status)})} className={`rounded-lg border px-2.5 py-1 text-[10px] font-extrabold ${editing?'cursor-pointer':'cursor-default'} ${STAGE_CLASSES[approval.status]}`} title={`${approval.name}: ${STAGE_LABELS[approval.status]}`}>{approval.name}<span className="ml-1 opacity-70">· {STAGE_LABELS[approval.status]}</span></button>)}</div>}
    </div>
