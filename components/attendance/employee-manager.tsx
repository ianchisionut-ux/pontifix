...
type Employee = { id: string; firstName: string; lastName: string; email: string | null; phone: string | null; position: string | null; department: string | null; category: Category; employmentType: string; weeklyHours: number; dailyHours: number; sortOrder: number; active: boolean; cnp: string | null; contractNumber: string | null; contractDate: string | Date | null; hiredAt: string | Date; endedAt: string | Date | null; grossSalary: number; baseFunction: boolean; dependents: number; personalDeduction: number; iban: string | null }
const EMPTY = { firstName: '', lastName: '', email: '', phone: '', position: '', department: '', category: 'PRODUCTIE' as Category, employmentType: 'FULL_TIME', weeklyHours: 40, dailyHours: 8, active: true, cnp: '', contractNumber: '', contractDate: '', hiredAt: new Date().toISOString().slice(0,10), endedAt: '', grossSalary: 0, baseFunction: true, dependents: 0, personalDeduction: 0, iban: '' }
...
export function EmployeeManager({ employees }: { employees: Employee[] }) {
  ...
  return <div>
    ...
    {editing && <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onMouseDown={() => setEditing(null)}><form action={submit} onMouseDown={(event) => event.stopPropagation()} className="bg-white rounded-3xl p-6 w-full max-w-3xl max-h-[92vh] overflow-y-auto shadow-2xl"><h2 className="text-xl font-semibold mb-5">{editing === 'new' ? 'Angajat nou' : 'Editează angajatul'}</h2><div className="grid sm:grid-cols-2 gap-3">
      ...
      <label className="text-sm font-medium">Data angajării<input className="input-field w-full mt-1.5" name="hiredAt" type="date" defaultValue={new Date(current.hiredAt).toISOString().slice(0,10)}/></label><label className="text-sm font-medium">Data încetării<input className="input-field w-full mt-1.5" name="endedAt" type="date" defaultValue={current.endedAt ? new Date(current.endedAt).toISOString().slice(0,10) : ''}/></label>
      <label className="text-sm font-medium">Salariu de bază brut<input className="input-field w-full mt-1.5" name="grossSalary" type="number" step="1" min="0" defaultValue={current.grossSalary}/></label>
