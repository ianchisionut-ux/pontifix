...
type Employee = { id: string; firstName: string; lastName: string; email: string | null; phone: string | null; position: string | null; department: string | null; employmentType: string; weeklyHours: number; active: boolean }
const EMPTY = { firstName: '', lastName: '', email: '', phone: '', position: '', department: '', employmentType: 'FULL_TIME', weeklyHours: 40, active: true }
...
export function EmployeeManager({ employees }: { employees: Employee[] }) {
  ...
  async function submit(formData: FormData) {
    ...
    const payload = { ...values, weeklyHours: Number(values.weeklyHours), active: formData.get('active') === 'on', ...(editing !== 'new' && editing ? { id: editing.id } : {}) }
