...
export function ConnectionsManager({ initialCases, identityCards, canManage, canEditDeerDate }: { initialCases: ConnectionCaseDto[]; identityCards: IdentityCardRecord[]; canManage: boolean; canEditDeerDate: boolean }) {
  ...
  return <div>
    ...
    {!canManage && viewMode === 'list' ? <AdminConnectionAccordion items={visible} identityCards={identityCards} query={query} onQueryChange={setQuery}/> : <div className={`grid min-h-[720px] overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm ${viewMode === 'list' ? 'xl:grid-cols-[300px_minmax(0,1fr)]' : 'grid-cols-1'}`}>
      ...
      <section className="min-w-0 p-5 lg:p-7">{selected ? <>
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 pb-5"><div><div className="flex items-center gap-2"><WandSparkles size={20} className="text-[#197fb5]"/><h2 className="text-xl font-bold text-[#082b4d]">{draft.Beneficiar || 'Fișă branșament'}</h2></div><div className="mt-2 flex flex-wrap items-center gap-2"><strong className="rounded-lg bg-[#edf7fc] px-2.5 py-1 text-xs text-[#0d5d8b]">{selected.nib}</strong><span className="rounded-lg px-2.5 py-1 text-xs font-bold text-white" style={{ backgroundColor: CONNECTION_STATUS_META[selected.status].color }}>{CONNECTION_STATUS_META[selected.status].label} · {CONNECTION_STATUS_META[selected.status].progress}%</span></div><p className="mt-1 text-xs text-slate-500">Creat de {selected.createdByEmail || 'administrator'} · actualizat {new Date(selected.updatedAt).toLocaleString('ro-RO')}</p>{notice && <p className="mt-2 text-sm font-semibold text-emerald-600">{notice}</p>}</div><div className="flex flex-wrap gap-2">{canManage && <select value={selected.status} disabled={busy === 'status'} onChange={(event) => changeStatus(event.target.value as ConnectionStatus)} className="input-field min-w-[210px] bg-white text-sm font-bold">{CONNECTION_STATUSES.map((status) => <option key={status} value={status}>{CONNECTION_STATUS_META[status].label}</option>)}</select>}<button type="button" onClick={()=>setCiOpen((value)=>!value)} className={`btn-secondary inline-flex items-center gap-2 ${ciOpen ? '!border-[#197fb5] !bg-[#edf7fc]' : ''}`}><CreditCard size={16}/> CI{identityCards.some((item)=>item.connectionCaseId===selected.id)&&<span className="h-2 w-2 rounded-full bg-emerald-500"/>}</button>{canManage && <>{selected.atrPathname && <a href={`/api/bransamente/${selected.id}/atr`} target="_blank" className="btn-secondary inline-flex items-center gap-2"><FileText size={16}/> ATR</a>}<a href={`/api/bransamente/${selected.id}/document?type=contract`} className="btn-secondary inline-flex items-center gap-2"><Download size={16}/> Contract + memoriu</a><a href={`/api/bransamente/${selected.id}/document?type=a3`} className="btn-secondary inline-flex items-center gap-2"><Download size={16}/> Dosar A3</a> </>}{canManage && <><button onClick={()=>openWhatsAppCenter('PROGRAMMED')} disabled={!!busy || !draft.Telefon.trim()} className="btn-secondary inline-flex items-center gap-2" title="Mesaje predefinite pentru beneficiar">{busy === 'whatsapp' ? <Loader2 size={16} className="animate-spin"/> : <MessageCircle size={16}/>} Mesaje WhatsApp</button><button onClick={save} disabled={!!busy} className="btn-primary inline-flex items-center gap-2">{busy === 'save' ? <Loader2 size={16} className="animate-spin"/> : <Save size={16}/>} Salvează</button><button onClick={remove} disabled={!!busy} className="round-action text-rose-500" title="Șterge dosarul"><Trash2 size={17}/></button></>}</div></div>
        {ciOpen && <div className="mb-6"><IdentityCardScanner records={identityCards} connections={items.map((item)=>({id:item.id,nib:item.nib,beneficiary:item.fields.Beneficiar||''}))} canEdit={canManage} connectionId={selected.id}/></div>}
