...
export function ConnectionsManager({ initialCases, canManage }: { initialCases: ConnectionCaseDto[]; canManage: boolean }) {
  ...
  const visible = useMemo(() => items.filter((item) => {
    const haystack = `${item.fields.Beneficiar} ${item.fields.Telefon} ${item.fields.ATR} ${item.fields.Amplasament}`.toLocaleLowerCase('ro-RO')
  ...
  return <div>
    ...
    <div className="grid min-h-[720px] overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm xl:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="border-b border-slate-200 bg-[#f5f9fc] p-4 xl:border-b-0 xl:border-r">
        ...
        <div className="space-y-2 xl:max-h-[620px] xl:overflow-y-auto xl:pr-1">{visible.map((item) => <button key={item.id} onClick={() => select(item)} className={`w-full rounded-2xl border p-3 text-left transition ${item.id === selectedId ? 'border-[#78bfe1] bg-white shadow-sm' : 'border-transparent bg-[#e9f1f8] hover:border-slate-200 hover:bg-white'}`}><p className="truncate text-sm font-black text-[#082b4d]">{item.fields.Beneficiar || 'Beneficiar necompletat'}</p><p className="mt-1 truncate text-xs text-slate-500">{item.fields.ATR || item.atrName || 'Fișă fără ATR'}</p><p className="mt-1 text-[11px] text-slate-400">{new Date(item.updatedAt).toLocaleString('ro-RO', { dateStyle: 'medium', timeStyle: 'short' })}</p></button>)}{!visible.length && <p className="rounded-2xl border border-dashed border-slate-300 p-5 text-center text-sm text-slate-500">Niciun dosar găsit.</p>}</div>
      </aside>
      <section className="min-w-0 p-5 lg:p-7">{selected ? <>
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 pb-5"><div><div className="flex items-center gap-2"><WandSparkles size={20} className="text-[#197fb5]"/><h2 className="text-xl font-bold text-[#082b4d]">{draft.Beneficiar || 'Fișă branșament'}</h2></div><p className="mt-1 text-xs text-slate-500">Creat de {selected.createdByEmail || 'administrator'} · actualizat {new Date(selected.updatedAt).toLocaleString('ro-RO')}</p>{notice && <p className="mt-2 text-sm font-semibold text-emerald-600">{notice}</p>}</div><div className="flex flex-wrap gap-2">{selected.atrPathname && <a href={`/api/bransamente/${selected.id}/atr`} target="_blank" className="btn-secondary inline-flex items-center gap-2"><FileText size={16}/> ATR</a>}<a href={`/api/bransamente/${selected.id}/document?type=contract`} className="btn-secondary inline-flex items-center gap-2"><Download size={16}/> Contract + memoriu</a><a href={`/api/bransamente/${selected.id}/document?type=a3`} className="btn-secondary inline-flex items-center gap-2"><Download size={16}/> Dosar A3</a>{canManage && <><button onClick={save} disabled={!!busy} className="btn-primary inline-flex items-center gap-2">{busy === 'save' ? <Loader2 size={16} className="animate-spin"/> : <Save size={16}/>} Salvează</button><button onClick={remove} disabled={!!busy} className="round-action text-rose-500" title="Șterge dosarul"><Trash2 size={17}/></button></>}</div></div>
        {!canManage && <div className="mb-5 flex items-center gap-2 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-semibold text-blue-800"><ShieldCheck size={18}/> Date protejate. Poți consulta și descărca documentele, iar modificările sunt rezervate Super Adminului.</div>}
