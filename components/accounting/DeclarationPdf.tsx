...
function Header({ company, report, type }: { company: Company; report: DeclarationReport; type: DeclarationType }) {
  return <>
    ...
    <Text style={styles.subtitle}>Interval date: {type === "D390" ? report.period.start : report.period.fiscalStart} - {report.period.endExclusive} (capat exclusiv).</Text>
    {report[type === "D300" ? "d300" : type === "D394" ? "d394" : "d390"].blockers.map(blocker => <Text key={blocker} style={styles.warning}>{pdfText(blocker)}</Text>)}
  </>;
}
...
export function DeclarationPdf({ company, report, type }: { company: Company; report: DeclarationReport; type: DeclarationType }) {
  return <Document title={`Fisa de lucru ${type} ${report.period.year}-${String(report.period.month).padStart(2,"0")}`}>
    <Page size="A4" orientation="landscape" style={styles.page}>
      ...
      {type==="D394"&&<><Text style={styles.section}>Livrari interne</Text><PartnerTable rows={report.d394.sales} empty="Nu exista livrari interne in perioada selectata."/><Text style={styles.section}>Achizitii interne - Intrari si Registru fiscal (documentele se numara o singura data)</Text><PartnerTable rows={report.d394.purchases.map(row=>({...row,taxableBase:row.net}))} empty="Nu exista achizitii interne inregistrate."/></>}
      {type==="D390"&&<><Text style={styles.section}>Livrari intracomunitare - tipul operatiei precede partenerul</Text><PartnerTable rows={report.d390.operations.filter(row=>row.direction==="SALE").map(row=>({...row,partnerName:`${row.operationCode||"Neclasificat"} - ${row.partnerName}`}))} empty="Nu exista livrari intracomunitare."/><Text style={styles.section}>Achizitii intracomunitare</Text><PartnerTable rows={report.d390.operations.filter(row=>row.direction==="PURCHASE").map(row=>({...row,partnerName:`${row.operationCode||"Neclasificat"} - ${row.partnerName}`}))} empty="Nu exista achizitii intracomunitare."/></>}
      <Footer/>
    </Page>
  </Document>;
}
