...
export function FormsManager({ initialForms, initialSubmissions, connections, projects, canManage }: {
  ...
  async function replaceForm(item: FormTemplateDto, replacement?: File) {
    ...
    setBusy(item.id)
    try {
      const blob = await uploadPdf(replacement, `formulare/${item.id}/${replacement.name}`)
  ...
  return <div>
    <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
      ...
      {canManage && <button className="btn-primary inline-flex items-center gap-2" onClick={() => setModalOpen(true)}><Plus size={17}/> {category === 'FORMULAR' ? tr('Adaugă formular') : tr('Adaugă model de cerere')}</button>}
    </header>
    ...
    {category === 'FORMULAR' ? <div className="grid gap-5 lg:grid-cols-2 2xl:grid-cols-3">{items.map((item) => <article key={item.id} className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm">
      ...
      <section><div className="mb-3 flex items-center gap-2"><FilePlus2 size={18} className="text-[#197fb5]"/><h2 className="font-bold text-[#082b4d]">{tr('Modele de cereri')}</h2></div><div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">{requests.map((item) => <article key={item.id} className="flex min-h-[150px] flex-col rounded-[20px] border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-auto flex flex-wrap items-center gap-2 pt-5"><button className="btn-primary inline-flex items-center gap-2 !px-4 !py-2 text-sm" onClick={() => setEditor({ template: item })}><Edit3 size={16}/> {tr('Completează pe PDF')}</button><a className="btn-secondary !px-3 !py-2 text-xs" href={`/api/formulare/${encodeURIComponent(item.id)}/document`} target="_blank">{tr('PDF gol')}</a>{canManage && <label className="btn-secondary inline-flex cursor-pointer items-center gap-2 !px-3 !py-2 text-xs"><RefreshCw size={14}/> {tr('Înlocuiește')}<input type="file" accept="application/pdf,.pdf" className="hidden" onChange={(event) => replaceForm(item, event.target.files?.[0])}/></label>}</div>
      </article>)}</div></section>
