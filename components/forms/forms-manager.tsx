...
type Category = 'FORMULAR' | 'CERERE' | 'CI'
...
export function FormsManager({ initialForms, initialSubmissions, connections, projects, identityCards, canManage, canEdit }: {
  ...
  return <div>
    <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
      ...
      {canManage && category !== 'CI' && <button className="btn-primary inline-flex items-center gap-2" onClick={() => { setBusy(''); setModalOpen(true) }}><Plus size={17}/> {category === 'FORMULAR' ? tr('Adaugă formular') : tr('Adaugă model de cerere')}</button>}
    </header>
...
<button onClick={() => setCategory('CI')} className={`rounded-xl px-5 py-2.5 text-sm font-bold ${category === 'CI' ? 'bg-[#0d5d8b] text-white' : 'text-slate-500 hover:bg-slate-50'}`}>CI <span className="ml-1 opacity-70">{identityCards.length}</span></button>
    </div>

    {category === 'CI' ? <IdentityCardScanner records={identityCards} connections={connections.map((item) => ({ id: item.id, nib: item.nib, beneficiary: item.fields.Beneficiar || '' }))} canEdit={canEdit}/> : category === 'FORMULAR' ? <div className="grid gap-5 lg:grid-cols-2 2xl:grid-cols-3">{items.map((item) => <article key={item.id} className="overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-sm">
