...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!process.env.RESEND_API_KEY) return NextResponse.json({ error: 'Serviciul de e-mail nu este configurat.' }, { status: 503 })
  ...
  const sent = await resend.emails.send({ from: process.env.RESEND_FROM_EMAIL || 'Elmont <onboarding@resend.dev>', to: data.customerEmail, subject: `Oferta Elmont ${data.offerNumber} – ${data.serviceType}`, html: offerEmailHtml(data) })
