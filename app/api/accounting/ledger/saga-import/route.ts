...
import { importSagaJournalEntries } from '@/lib/accounting/ledger'
...
async function POSTHandler(req:NextRequest){
  try{
    ...
    const preview={total:parsed.entries.length,errors:parsed.errors,entries:parsed.entries.slice(0,50)}
    if(form.get('commit')!=='1')return NextResponse.json(preview)
    if(parsed.errors.length)return NextResponse.json({error:'Corectează erorile din previzualizare înainte de import.',...preview},{status:400})
